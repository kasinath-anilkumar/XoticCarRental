...
test("the city directory combines configured state selection and search", async ({ page }) => {
  ...
  const cards = page.getByRole("region", { name: "City directory", exact: true }).getByRole("article");
  ...
  const search = page.getByRole("searchbox", { name: "Search service cities or airports" });
